...
export default async function Wakatime() {
  ...
  const editors = stats?.data?.editors ?? [];
  const aiModelBreakdown = stats?.data?.ai_model_breakdown ?? [];
  const aiAgentTotalCost = aiModelBreakdown.reduce(
    (acc, agent) => acc + agent.cost,
    0,
  );
    ...
    <section className="w-full space-y-8">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-5">
        ...
        <Card>
          ...
          <p className="mt-1 text-sm">
            {aiModelBreakdown[0]
              ? `$${aiModelBreakdown[0].cost.toFixed(2)}`
              : "—"}
          </p>
        </Card>
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-3">
        {aiModelBreakdown.slice(0, 8).map((agent, index) => (
          <div
            key={index}
            data-aos="fade-up"
            ...
            <Card>
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-xs">${agent.cost.toFixed(2)}</p>
                </div>
              </div>
                ...
                <div className="h-1.5 w-full overflow-hidden rounded-full">
                  <div
                    ...
                    style={{
                      width: `${Math.min(
                        100,
                        aiModelBreakdown[0]?.lines
                          ? (agent.lines / aiModelBreakdown[0].lines) * 100
                          : 0,
                      )}%`,
                    }}
                  />
